...
export default function QRPassScreen() {
  ...
  const loadPassFromStorage =
    useCallback(async (): Promise<CachedPass | null> => {
  ...
  const fetchOtp = useCallback(
    async (passJti: string): Promise<string | null> => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const rawJti =
        ...
        (body?.data &&
        typeof (body.data as Record<string, unknown>)?.jti === "string"
      ...
      if (cached) {
        ...
        setLoadError(
          "Could not load pass. Please check your connection and try again.",
        );
      }
    } finally {
      setLoading(false);
  ...
  const loadDataByJti = useCallback(async () => {
    ...
    try {
      ...
      const tripIdFromPass =
        passBody?.tripId != null ? String(passBody.tripId) : null;
      const otpBody = otpRes.data as Record<string, unknown> | undefined;
      const newOtp = typeof otpBody?.otp === "string" ? otpBody.otp : null;
      setJti(passJti);
      setOtp(newOtp);
        ...
        <View style={styles.quickActions}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              Alert.alert(
                "Add to Wallet",
                "Apple Wallet integration is coming soon.",
              )
            }
            activeOpacity={0.88}
          >
        ...
        <View style={[styles.hintsCard, CARD_SHADOW]}>
          ...
          <View style={styles.hintRow}>
            ...
            <Text style={styles.hintText}>
              Present this QR code at airport security, hotel front desk, or
              transport gates.
            </Text>
          </View>
          <View style={styles.hintRow}>
